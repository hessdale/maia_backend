...
begin
	...
	token_input,now());
	select id,convert(token using"utf8")as token from client_session where id = last_insert_id(); 
	commit;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
begin
    ...
    select dp.file  from maia_db.design_photos dp where id=last_insert_id();
    commit;
    end if;
END ;;
DELIMITER ;
...
begin
    ...
    select mp.file  from maia_db.misc_photos mp where id=last_insert_id();
    commit;
    end if;
END ;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `post_portrait_photo`(token_input varchar(100),file_input varchar(100),image_portrait_input varchar(100))
    MODIFIES SQL DATA
begin
	if (select id from client_session where token=token_input) is not null then
	INSERT INTO maia_db.portrait_photos (file, image_description) VALUES(file_input,image_description_input);
    select pp.file  from maia_db.portrait_photos pp where id=last_insert_id();
    commit;
    end if;
END ;;
DELIMITER ;
...
-- Dump completed on 2024-02-12 17:37:19
